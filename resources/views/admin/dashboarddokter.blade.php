...
<head>
  ...
  <title>Dashboard Dokter - RSUD Banyuasin</title>
  <!-- Fonts and icons -->
  ...
  <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-radius-lg fixed-start ms-2 bg-white my-2" id="sidenav-main">
    ...
    <div class="collapse navbar-collapse w-auto" id="sidenav-collapse-main">
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link active bg-gradient-dark text-white" href="/admin/dashboarddokter">
        ...
        <li class="nav-item">
          <a class="nav-link text-dark" href="/admin/dashboardberita">
      ...
      <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
            ...
            <li class="breadcrumb-item text-sm text-dark active" aria-current="page">Dashboard Dokter</li>
          </ol>
        </nav>
          ...
          <div class="card my-4">
            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
              <div class="bg-gradient-dark shadow-dark border-radius-lg pt-4 pb-3">
                <h6 class="text-white text-capitalize ps-3">Doctor Management</h6>
              </div>
            </div>
            <div class="card-body px-0 pb-2">
              <div class="table-responsive p-0">
                <a href="{{ route('admin.dokter.create') }}" class="btn btn-primary mb-3 ms-3">Add New Doctor</a>
                <table class="table align-items-center mb-0">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td class="ps-4">{{ $dokter->nama }}</td>
                      <td>{{ $dokter->spesialisasi }}</td>
                      <td class="align-middle text-center">{{ $dokter->jadwal }}</td>
                      <td class="align-middle text-center">
                        <a href="{{ route('admin.dokter.show', $dokter->id) }}" class="btn btn-info btn-sm px-3 py-1 me-1">View</a>
                        <a href="{{ route('admin.dokter.edit', $dokter->id) }}" class="btn btn-warning btn-sm px-3 py-1 me-1">Edit</a>
                        <form action="{{ route('admin.dokter.destroy', $dokter->id) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger btn-sm px-3 py-1" onclick="return confirm('Are you sure you want to delete this doctor?')">Delete</button>
                        </form>
                      </td>
                    </tr>
                    @endforeach
